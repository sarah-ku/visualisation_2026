...
* Anvende `nest()` til at samle deldatasæt i en liste-kolonne og derefter bruge `map` til at gentage en bestemt analyse på tværs af flere deldatasæt uden at copy/paste koder.
* Bruge funktionen `geom_smooth()` til at visualisere lineær regression eller loess-kurver.
* Omdanne output fra statistiske tests og modeller til tidy tibbles med `broom`-pakken (fx `glance()` og `tidy()`)
* bruge anova() til at sammenligne indlejrede (nested) modeller, der er fit’et på samme datasæt.
* (valgfrit) tilføje modelstatistikker og signifikans-annotationer til figurer med `glue`-pakken og `ggsignif()`.
:::
...
* Video 2: Lineær regression linjer med ggplot2
  ...
  + Jeg sammenligner linjen med resultatet fra `lm()`
...
* Video 3: Lineær regression med `nest()` og `map()`
  + Den process igen fra Video 1 men anvendte på lineær regression
...
```{r}
library(broom) 
my_cor_test <- ~cor.test(.x$gc,log10(.x$size_mb)) %>% broom::glance()
```
...
## Plot linear regression estimates
...
| Input til `.x` i `map(.x, .f)`                                       | Hvad `.x` er inde i funktionen | Output fra `map()`                      | Eksempel                                 |
| -------------------------------------------------------------------- | ------------------------------------------------------------------------ | --------------------------------------- | --------------------------------------------- |
...
| Nested tibble med en liste-kolonne fra `nest()` (fx kolonnen `data`) | Ét gruppe-opdelt data frame ad gangen (ét element i liste-kolonnen)      | Liste-kolonne med én entry pr. gruppe   | `nested %>% mutate(n = map(data, nrow))`      |
...
* Brug `data(mtcars)` og `cor.test()` til at udføre en test af korrelationen mellem variablerne `qsec` og `drat` (OBS: uden `group_by()` og `nest()` her). 
...
* Hvad er skæringspunktet og hældningen af den beregnede linje?
